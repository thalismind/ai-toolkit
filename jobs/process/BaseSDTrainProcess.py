...
from toolkit.config_modules import SaveConfig, LoggingConfig, SampleConfig, NetworkConfig, TrainConfig, ModelConfig, \
    ...
    DecoratorConfig, ValidateConfig
...
class BaseSDTrainProcess(BaseTrainProcess):
    ...
    def __init__(self, process_id: int, job, config: OrderedDict, custom_pipeline=None):
        ...
        self.validate_config = ValidateConfig(**self.get_conf('validate', {}))
        ...
        validate_configs(self.train_config, self.model_config, self.save_config, self.validate_config, self.dataset_configs)
    ...
    def run(self):
        ...
        if self.datasets is not None:
            self.data_loader, self.validation_data_loader = get_dataloader_from_datasets(self.datasets, self.train_config.batch_size, self.sd)
        if self.datasets_reg is not None:
            self.data_loader_reg, self.validation_data_loader_reg = get_dataloader_from_datasets(self.datasets_reg, self.train_config.batch_size,
                                                                self.sd)
